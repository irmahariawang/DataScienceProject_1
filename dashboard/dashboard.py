...
# Helper function yang dibutuhkan untuk menyaipkan berbagai dataframe
...
def create_bycity_df(df):
    bycity_df = df.groupby(by="customer_city").customer_unique_id.nunique().sort_values(ascending=False).reset_index()
...
with st.sidebar:
    ...
    st.image("https://github.com/irmahariawang/Source/blob/cf9bd95a53be6af56eb057648d597c83a541b8dc/logo-color.png")
...
st.header('Irma Submission Dashboard :sparkles:')
...
with col2:
    total_revenue = format_currency(daily_orders_df.revenue.sum(), "REAL", locale='es_CO') 
...
with col3:
    avg_frequency = format_currency(rfm_df.monetary.mean(), "REAL", locale='es_CO') 
...
ax[2].set_xticklabels(ax[2].get_xticklabels(), rotation=45, horizontalalignment='right', fontsize=12)
ax[2].tick_params(axis='y', labelsize=30)
ax[2].tick_params(axis='x', labelsize=35)

st.pyplot(fig)

st.caption('Irma 2024')